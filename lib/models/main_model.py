...
def get_backbone(backbone='resnet34', pretrained=True):
    if backbone == 'resnet34':
        A = torch.nn.Sequential(*list(resnet34(pretrained=pretrained).children())[:-2])
        B = torch.nn.Sequential(*list(resnet34(pretrained=pretrained).children())[:-3])
        C = torch.nn.Sequential(*list(resnet34(pretrained=pretrained).children())[:-4])
        D = torch.nn.Sequential(*list(resnet34(pretrained=pretrained).children())[:-5])
    return A, B, C, D

A, B, C, D = get_backbone()
A = A.to('cuda')
B = B.to('cuda')
C = C.to('cuda')
D = D.to('cuda')
...
class main_model(nn.Module):
    ...
    def __init__(self, num_classes, patch_size=4, in_chans=3, embed_dim=96, depths=(2, 2, 2, 2),
        ...
        self.conv1 = nn.Conv2d(64, 96, kernel_size=1).to('cuda')
        self.conv2 = nn.Conv2d(128, 192, kernel_size=1).to('cuda')
        self.conv3 = nn.Conv2d(256, 384, kernel_size=1).to('cuda')
        self.conv4 = nn.Conv2d(512, 768, kernel_size=1).to('cuda')
        self.conv5 = nn.Conv2d(768, 64, kernel_size=1).to('cuda')
    @staticmethod
    ...
    def forward(self, imgs):
        ...
        x_s_1 = x_s_1.reshape(x_s_1.shape[0], -1, 96, 160)
        x_s_2 = torch.transpose(x_s_2, 1, 2)
        x_s_2 = x_s_2.reshape(x_s_2.shape[0], -1, 48, 80)
        x_s_3 = torch.transpose(x_s_3, 1, 2)
        x_s_3 = x_s_3.reshape(x_s_3.shape[0], -1, 24, 40)
        x_s_4 = torch.transpose(x_s_4, 1, 2)
        x_s_4 = x_s_4.reshape(x_s_4.shape[0], -1, 12, 20)
        ...
        x_f_4 = self.fu4(x_c, x_s_4, x_f_3)
        x_f_4 = self.conv5(x_f_4)
